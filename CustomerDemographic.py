...
# reading the CustomerDprint(df)emograohic sheet from the excel file
...
#functio to label the plots for each bar plot
def autolabel(rects):
    for rect in rects:
        h = rect.get_height()
...
df.loc[df['DOB'].isnull(), 'DOB'] = datetime.datetime.now()
df[["DOB"]] = df[["DOB"]].astype(str)
for i in range(1, len(df["DOB"])+1):
    if isinstance(df["DOB"][i], str):
        df.loc[df['DOB'].isnull(), 'DOB'] = datetime.datetime.now()
        #print(reference_year)
        date = pd.Series(df["DOB"][i])
        year = date.str.slice(0, 4).astype(np.float)
        month = date.str.slice(5, 7).astype(np.float)
        df["Age"][i] = np.floor((12 * (reference_year - year) + (reference_month - month)) / 12)
# print(df["Age"])
#print(df)
...
#print ("first quater", firtQ , "second quarter", secondQ, "third quarter", thirdQ, "fourth quarter", fourthQ)
i = 1
for items in df["gender"]:
    tAge = int (df["Age"][i])
